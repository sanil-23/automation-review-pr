...
function renderPrDetail(pr, container) {
  ...
  container.innerHTML = `
    ...
    <div class="section" id="gh-description-section">
      <div class="section-header">
        <h3>Description</h3>
        <span class="badge badge-gray" id="gh-description-status">Loading from GitHub…</span>
      </div>
      <div id="gh-description-content" class="md-content"></div>
    </div>
    ...
    <div class="section">
      <div class="section-header">
        <h3>Files Changed</h3>
        <span class="badge badge-gray" id="gh-files-status">Loading from GitHub…</span>
      </div>
      <div id="gh-files-content"></div>
    </div>

    <div class="section">
      <div class="section-header">
        <h3>Comments</h3>
        <span class="badge badge-gray" id="gh-comments-status">Loading from GitHub…</span>
      </div>
      <div id="gh-comments-content"></div>
    </div>
  ...
  loadLogs(pr.id);
  loadGithubData(pr.id);
}

// Fetch PR description, files, and comments directly from GitHub's public API.
// Repo is public so the browser can hit api.github.com unauthenticated. Rate
// limit is 60 req/hr per IP — fine for clicking through individual PRs.
const GH_REPO = 'tinyhumansai/openhuman';
const GH_API = `https://api.github.com/repos/${GH_REPO}`;

async function loadGithubData(prId) {
  loadGithubDescription(prId);
  loadGithubFiles(prId);
  loadGithubComments(prId);
}

async function loadGithubDescription(prId) {
  const content = document.getElementById('gh-description-content');
  const status = document.getElementById('gh-description-status');
  if (!content) return;
  try {
    const res = await fetch(`${GH_API}/pulls/${prId}`);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    if (!data.body || data.body.trim() === '') {
      content.innerHTML = '<p style="color:var(--text-muted)">No description.</p>';
      status.textContent = 'Empty';
      return;
    }
    content.innerHTML = window.marked ? marked.parse(data.body) : `<pre>${esc(data.body)}</pre>`;
    status.textContent = 'GitHub';
  } catch (err) {
    content.innerHTML = `<p style="color:var(--text-muted)">Could not load description: ${esc(err.message)}</p>`;
    status.textContent = 'Error';
  }
}

async function loadGithubFiles(prId) {
  const content = document.getElementById('gh-files-content');
  const status = document.getElementById('gh-files-status');
  if (!content) return;
  try {
    const res = await fetch(`${GH_API}/pulls/${prId}/files?per_page=100`);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const files = await res.json();
    if (!Array.isArray(files) || files.length === 0) {
      content.innerHTML = '<p style="color:var(--text-muted)">No files.</p>';
      status.textContent = '0 files';
      return;
    }
    const rows = files.map((f) => {
      const statusMap = { added: 'badge-green', removed: 'badge-red', modified: 'badge-gray', renamed: 'badge-purple' };
      const badge = `<span class="badge ${statusMap[f.status] || 'badge-gray'}">${f.status}</span>`;
      return `
        <tr>
          <td><code>${esc(f.filename)}</code></td>
          <td>${badge}</td>
          <td style="color:var(--green)">+${f.additions}</td>
          <td style="color:var(--red)">-${f.deletions}</td>
          <td><a href="${esc(f.blob_url || '')}" target="_blank" class="btn btn-sm">View</a></td>
        </tr>`;
    }).join('');
    content.innerHTML = `
      <div class="pr-table-wrap">
        <table>
          <thead><tr><th>File</th><th>Status</th><th>+</th><th>-</th><th></th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>`;
    status.textContent = `${files.length} file${files.length === 1 ? '' : 's'}`;
  } catch (err) {
    content.innerHTML = `<p style="color:var(--text-muted)">Could not load files: ${esc(err.message)}</p>`;
    status.textContent = 'Error';
  }
}

async function loadGithubComments(prId) {
  const content = document.getElementById('gh-comments-content');
  const status = document.getElementById('gh-comments-status');
  if (!content) return;
  try {
    // Issue comments (general discussion) + PR review comments (inline on diff)
    const [issueRes, reviewRes] = await Promise.all([
      fetch(`${GH_API}/issues/${prId}/comments?per_page=100`),
      fetch(`${GH_API}/pulls/${prId}/comments?per_page=100`),
    ]);
    const issueComments = issueRes.ok ? await issueRes.json() : [];
    const reviewComments = reviewRes.ok ? await reviewRes.json() : [];
    const all = [
      ...issueComments.map((c) => ({ ...c, kind: 'issue' })),
      ...reviewComments.map((c) => ({ ...c, kind: 'review' })),
    ].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));

    if (all.length === 0) {
      content.innerHTML = '<p style="color:var(--text-muted)">No comments.</p>';
      status.textContent = '0 comments';
      return;
    }

    content.innerHTML = all.map((c) => {
      const body = window.marked ? marked.parse(c.body || '') : `<pre>${esc(c.body || '')}</pre>`;
      const where = c.kind === 'review' && c.path ? ` <code style="font-size:12px">${esc(c.path)}${c.line ? ':' + c.line : ''}</code>` : '';
      return `
        <details style="margin-bottom:10px" open>
          <summary style="cursor:pointer;padding:8px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:var(--radius)">
            <strong>${esc(c.user?.login || 'unknown')}</strong>
            <span style="color:var(--text-muted);font-size:13px">${timeAgo(c.created_at)}</span>
            ${where}
            <a href="${esc(c.html_url || '')}" target="_blank" class="btn btn-sm" style="float:right">Open</a>
          </summary>
          <div class="md-content" style="padding:10px;border:1px solid var(--border);border-top:0;border-radius:0 0 var(--radius) var(--radius)">${body}</div>
        </details>`;
    }).join('');
    status.textContent = `${all.length} comment${all.length === 1 ? '' : 's'}`;
  } catch (err) {
    content.innerHTML = `<p style="color:var(--text-muted)">Could not load comments: ${esc(err.message)}</p>`;
    status.textContent = 'Error';
  }
}
